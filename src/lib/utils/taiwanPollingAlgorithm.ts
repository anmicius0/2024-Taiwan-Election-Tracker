...
export class TaiwanPollingAlgorithm {
	...
	public predictElection(
		...
		for (const poll of pollingData) {
			...
			// Weights (Exponential Half-Life Decay)
			const recencyWeight = Math.pow(0.5, daysBeforeElection / this.RECENCY_HALFLIFE);
			...
			const methodKey = Object.keys(this.METHOD_WEIGHTS).find((key) => pollMethod.includes(key));
			const methodWeight = methodKey
				? this.METHOD_WEIGHTS[methodKey]
				: this.METHOD_WEIGHTS['unknown'];
	...
	public evaluatePrediction(
		...
		const calcError = (pred: number, act: number) => Number(Math.abs(pred - act).toFixed(2));

		const dpp_error = calcError(p.dpp, actual.dpp);
		const kmt_error = calcError(p.kmt, actual.kmt);
		const tpp_error = calcError(p.tpp, actual.tpp);

		return {
			dpp_error,
			kmt_error,
			tpp_error,
			total_error: Number((dpp_error + kmt_error + tpp_error).toFixed(2))
		};
	}
}
